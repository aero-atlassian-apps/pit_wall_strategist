...
export class IssueCategorizer {
    ...
    inferBlockingReason(issue: JiraIssue): string {
        ...
        if (labels.some(l => l.toLowerCase().includes('block'))) return 'Explicitly marked as blocked';
        ...
        if (labels.some(l => l.toLowerCase().includes('review'))) return 'Stuck in code review';
