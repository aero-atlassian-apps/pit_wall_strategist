...
export async function fetchBoardData(projectKey: string, config: TelemetryConfig = DEFAULT_CONFIG, context?: any): Promise<BoardData> {
    ...
    if (!security.permissions.userBrowse && !security.permissions.appBrowse) {
        console.warn(`[Telemetry] Access Denied for ${projectKey}. (User Browse: ${security.permissions.userBrowse}, App Browse: ${security.permissions.appBrowse})`);
        console.warn(`[Telemetry] Reasons: ${security.messages.join(' | ')}`);
        return {
            boardType: 'business',
            boardId: null,
