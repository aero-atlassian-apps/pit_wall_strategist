...
function formatMarkdown(
    ...
    const now = new Date()
    const dateStr = now.toLocaleDateString((locale || 'en'), { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
    ...
    let md = `# 🏎️ ${t('dailyStandupReport', locale)}\n`
    ...
    if (telemetry) {
        ...
        md += `## 📊 ${t('raceTelemetry', locale)}\n`
        md += `- **${t('status', locale)}:** ${statusEmoji} ${telemetry.sprintStatus || t('unknown', locale)}\n`
        md += `- **${t('wipLoad', locale)}:** ${telemetry.wipCurrent || 0} / ${telemetry.wipLimit || 8} (${Math.round(((telemetry.wipCurrent || 0) / (telemetry.wipLimit || 8)) * 100)}%)\n`
        md += `- **${t('velocityDelta', locale)}:** ${(telemetry.velocityDelta || 0) >= 0 ? '+' : ''}${telemetry.velocityDelta || 0}%\n`
        md += `\n`
    }

    // Stalled Tickets (Blockers)
    if (stalledTickets.length > 0) {
        md += `## 🚨 ${t('redFlagsStalled', locale)}\n`
        for (const ticket of stalledTickets) {
            md += `- **${ticket.key}**: ${ticket.summary || t('noSummary', locale)}\n`
            md += `  - ${t('assignee', locale)}: ${ticket.assignee || t('unassigned', locale)}\n`
            md += `  - ${t('stalled', locale)}: ${ticket.hoursSinceUpdate || 0}h\n`
        }
        md += `\n`
    }

    // Who's Working on What
    md += `## 👥 ${t('driverAssignments', locale)}\n`
    for (const [assignee, assigneeIssues] of Object.entries(byAssignee)) {
        ...
        if (inProgress.length > 0) {
            md += `### ${assignee} (${inProgress.length} ${t('active', locale)})\n`
    ...
    // Quick Stats
    md += `## 📈 ${t('quickStats', locale)}\n`
    md += `- **${t('totalIssues', locale)}:** ${issues.length}\n`
    md += `- **${t('stalled', locale)}:** ${stalledTickets.length}\n`
...
export default function StandupExportButton({
    ...
    const handleExport = useCallback(() => {
        ...
        const markdown = formatMarkdown(issues, stalledTickets, sprintName, telemetry, locale)

        // Copy to clipboard
        navigator.clipboard.writeText(markdown).then(() => {
            alert(`📋 ${t('standupCopied', locale)}`)
        }).catch(() => {
    ...
    return (
        <Button onClick={handleExport}>
            <Icon>📋</Icon>
            {t('exportStandup', (window as any).__PWS_LOCALE || 'en')}
        </Button>
    )
}
